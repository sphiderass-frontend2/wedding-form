...
export default function Preview({ formData, onSave, isSaving = false }: Props) {
  return (
      ...
      <div className="bg-tab-primary p-5 rounded-2xl">
        ...
        {formData.guestList?.length > 0 ? (
          formData.guestList.map((guest: any, index: number) => (
            <div
              key={index}
              className="p-4 rounded-2xl bg-tab-secondary flex justify-between items-center text-text-primary mt-3"
            >
